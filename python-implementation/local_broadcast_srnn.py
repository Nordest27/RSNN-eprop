...
class LocalBroadcastSrnn:
        ...
        for i in range(self.num_layers):
            p = mp.Process(
                ...
                kwargs=dict(
                    ...
                    right_input_queue=self.input_queues[i+1] if i < self.num_layers-1 else None,
    ...
    def lif_worker(
        ...
        how_many_pulses = 0
        while True:
            # instruction, data = input_queue.get(timeout=np.random.rand()/100)
            # print(time.time_ns() - last_time)
            # if time.time_ns() - last_time  > 1e6:
            #     how_many += 1
            #     # input_queue.put(("NEXT STEP", None))
            #     last_time = time.time_ns()
            ...
            match instruction:
                ...
                case "PULSE-R":
                    ...
                    how_many_pulses += 1
                    if left_input_queue is not None:
                        left_input_queue.put(("PULSE-R", data))
                    ...
                    if left_input_queue is not None:
                        left_input_queue.put(("PULSE-L", output_spikes))
                    if left_input_queue is not None:
                        left_input_queue.put(("PULSE-R", output_spikes))
                    output_queue.put((layer_id, output_spikes))

                case "PULSE-L":
                    layer.receive_pulse(data)
                    how_many_pulses += 1
                    if right_input_queue is not None:
                        right_input_queue.put(("PULSE-L", data))
                    ...
                    if right_input_queue is not None:
                        right_input_queue.put(("PULSE-L", output_spikes))
                    if left_input_queue is not None:
                        left_input_queue.put(("PULSE-R", output_spikes))
                    output_queue.put((layer_id, output_spikes))
    ...
    def input(self, input_data):
        ...
        # self.output_layer.receive_pulse(self.output_queue.get()[1])
